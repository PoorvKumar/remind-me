...
const UpcomingReminders: React.FC<UpcomingRemindersProps> = ({
  ...
  return (
    <div className="space-y-4 mt-4 ">
      ...
      <div
        className={`${reminders.length ? "max-h-64" : "h-10"} overflow-y-auto custom-scrollbar`}
      >
        {reminders.length === 0 ? (
          ...
          reminders.map((reminder) => (
            ...
            >
              {reminder.status === "overdue" && (
                <div
                  className="absolute top-0 left-0 bg-red-500 text-white text-xs rounded-lg pl-4 pr-3 py-1 transform  shadow-md"
                  style={{
                    transform:
                      "rotate(-30deg) translateX(-25%) translateY(10%)",
                  }}
                >
                  Overdue
                </div>
